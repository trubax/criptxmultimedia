              ...
              snapshot.docs.map(async (docSnapshot) => {
                ...
                if (isNewMessage && data.senderId !== currentUser.uid) {
                  ...
                  if (chatDoc.exists()) {
                    ...
                    const currentUnread = unreadCount[currentUser.uid] || 0;
                    
                    await updateDoc(chatRef, {
                      [`unreadCount.${currentUser.uid}`]: currentUnread + 1,
                      lastMessageTime: serverTimestamp() // Aggiorna il timestamp per l'ordinamento
                    });
                  }
                }
